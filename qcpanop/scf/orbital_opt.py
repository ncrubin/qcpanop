...
def orbital_optimize_spinorb(reduced_ham: of.InteractionOperator,
    ...
    while grad_residual > grad_eps and current_iter < maxiter:
        comm_grad = np.zeros(num_vars, dtype=tpdm.dtype)
        ...
        comm_hess = np.zeros((num_vars, num_vars), dtype=tpdm.dtype)
...
def oo_so_sep(obi: np.ndarray, tbi: np.ndarray,
              ...
              verbose: bool=False, method='newton-raphson') -> np.ndarray:
    """
    Implement orbital rotation from specification of the spatial integrals
...
def spatial_oo(oei: np.ndarray, tei: np.ndarray, sopdm: np.ndarray,
    ...
    current_iter = 0
    while grad_residual > grad_eps and current_iter < maxiter:
        comm_grad = np.zeros(num_vars, dtype=opdm.dtype)
        ...
        # switch to full newton once the gradient is small
        if grad_residual < 1.0E-3:
            stepper = Stepper('newton-raphson')

        comm_hess = np.zeros((num_vars, num_vars), dtype=opdm.dtype)
        for idx, (v1, o1) in enumerate(zip(ridx, cidx)):
            for jdx, (v2, o2) in enumerate(zip(ridx, cidx)):
                if idx >= jdx:
                    hess_val = 0.
