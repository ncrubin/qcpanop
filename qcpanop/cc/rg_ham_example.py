...
def main():
    ...
    for g in couplings:
        ...
        assert test1 == test2  # check if equivalent to the original Hamiltonian
        ...
        print()
        # solve CCD equations
        cc_energy, lagrangian_energy, cc_opdm, cc_tpdm, cc_t2f, cc_l2f = solve_cc_equations(
            ncr_h1.astype(float), 0.5 * ncr_h2_antisymm.astype(float))
        ccd_energies.append(cc_energy)

        doci_energies.append(rg_eigs)
    ...
    ax.plot(couplings, ccd_energies, 'C0o', label='CCD')
    ...
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(6.4, 4.8))
    ax.plot(couplings, np.array(ccd_energies).flatten() - qubit_doci_spectrum[:, 0].flatten(), 'C0o-', label='CCD')
    ax.tick_params(which='both', labelsize=18, direction='in')
    ax.set_xlabel("g", fontsize=18)
