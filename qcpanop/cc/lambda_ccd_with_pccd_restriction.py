"""
Courtesy of pdaggerq

A full working spin-orbital lambda-CCD code generated with pdaggerq

If you want to run the example here you should install pyscf openfermion and\
openfermion-pyscf. The actual CCSD code (ccsd_energy, singles_residual,\
doubles_residual, kernel) do not depend on those packages,\
but you gotta get integrals frome somehwere.
...
Here we allow the restriction to pair doubles amplitudes in the CCD code. This
is less efficient but useful.
"""
...
class LambdaCCD:
    ...
    def __init__(
            ...
            *,
            ...
            restrict_to_pair_doubles=False
    ):
        ...
        self.cc_energy = None
        self.lagrangian_energy = None
        self.cc_opdm = None
        self.cc_tpdm = None
        self.cc_t2f = None
        self.cc_l2f = None
        self.restrict_to_pair_doubles = restrict_to_pair_doubles

    def solve_cc_equations(self, soei, astei):
        """Lambda-CCD equations are solved. Return amplitudes and RDMs"""
        nso = soei.shape[0]
        nsocc = self.molecule.n_electrons
        nsvirt = nso - nsocc
        n = np.newaxis
        o = slice(None, nsocc)
        v = slice(nsocc, None)
        ...
        t1z, t2z = np.zeros((nsvirt, nsocc)), np.zeros(
            (nsvirt, nsvirt, nsocc, nsocc))
        t1f, t2f, l1f, l2f = kernel(
            t1z,
            t2z,
            fock,
            g,
            ...
            stopping_eps=self.e_convergence,
            # r_convergence=self.r_convergence,
            restrict_to_pair_doubles=self.restrict_to_pair_doubles
        )
        ...
        final_lagrangian_energy = lagrangian_energy(t1f, t2f, l1f, l2f, fock, g,
                                                    o, v) - hf_energy
        print(
            "{: 5.15f} Lagrangian Energy - HF".format(final_lagrangian_energy))
        ...
        return final_cc_energy + hf_energy, \
            final_lagrangian_energy + hf_energy, opdm, tpdm, t2f, l2f

    def compute_energy(self):
        ncr_h1, ncr_h2 = get_tensors_from_integrals(
            self.molecule.one_body_integrals, self.molecule.two_body_integrals)

        ncr_h2 = ncr_h2 - np.einsum('ijlk', ncr_h2)  # antisymmetrize
        ncr_h2 *= 4  #compensate for factor of 4 in 2-body integral convention

        self.cc_energy, self.lagrangian_energy, self.cc_opdm, self.cc_tpdm, \
            self.cc_t2f, self.cc_l2f = self.solve_cc_equations(
            ncr_h1.astype(float), 0.5 * ncr_h2.astype(float))
        return self.cc_energy
...
def kernel(t1, t2, fock, g, o, v, e_ai, e_abij, max_iter=100,
           stopping_eps=1.0E-8, restrict_to_pair_doubles=False):
    """
    ...
    if restrict_to_pair_doubles:
        pair_doubles_mask = np.zeros_like(t2)
        nocc = o.stop
        nvirt = fock.shape[0] - nocc
        for a in range(nvirt//2):
            for i in range(nocc//2):
                # alpha-beta, beta-alpha
                pair_doubles_mask[2 * a, 2 * a + 1, 2 * i, 2 * i + 1] = 1
                pair_doubles_mask[2 * a + 1, 2 * a, 2 * i + 1, 2 * i] = 1

                # these amplitudes will be negative of the ones above
                # but the still need be included
                pair_doubles_mask[2 * a + 1, 2 * a, 2 * i, 2 * i + 1] = 1
                pair_doubles_mask[2 * a, 2 * a + 1, 2 * i + 1, 2 * i] = 1
    ...
    for idx in range(max_iter):
        ...
        residual_singles = np.zeros_like(t1)
        t1 *= 0
        residual_doubles = doubles_residual(t1, t2, fock, g, o, v)
        if restrict_to_pair_doubles:
            residual_doubles *= pair_doubles_mask
        ...
        if restrict_to_pair_doubles:
            doubles_res *= pair_doubles_mask

        new_singles = singles_res * e_ai
        new_singles *= 0
        new_doubles = doubles_res * e_abij
        if restrict_to_pair_doubles:
            new_doubles *= pair_doubles_mask
    ...
    if restrict_to_pair_doubles:
        lpair_doubles_mask = pair_doubles_mask.transpose(2, 3, 0, 1)
    ...
    for idx in range(max_iter):
        ...
        lsingles_res = np.zeros_like(l1)
        ldoubles_res = lambda_doubles(t1, t2, l1, l2, fock, g, o, v)
        if restrict_to_pair_doubles:
            ldoubles_res *= lpair_doubles_mask
        ...
        if restrict_to_pair_doubles:
            lnew_doubles *= lpair_doubles_mask
