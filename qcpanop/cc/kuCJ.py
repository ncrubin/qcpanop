...
class RkuCJ:
    ...
    def __init__(self, norb: int, k_layers: int):
        """
        A slightly modified restricted k-unitary Cluster Jastrow of
        https://doi.org/10.1021/acs.jctc.9b00963.  The modification is that
        we parameterize each layer with a one-body rotation and charge-charge
        and thus each layer is e^{\kappa}e^{ninj} --i.e. a bais rotation
        followed by a charge-charge like network.
