...
def test_uccsd_vs_rccsd():
    ...
    mf = mf.to_uhf()
    out, out_t = run_ucc(hcore, hcore, chem_eri, chem_eri, chem_eri, nalpha, nbeta, mf, True)
...
def test_uccsd_vs_uccsd():
    ...
    out, out_t = run_ucc(ha, hb, chem_eriaa, chem_eribb, chem_eriab, nalpha, nbeta, mf, True)
    assert abs(ref - out) < 1e-6
    assert abs(ref_t - out_t) < 1e-6
